...
Forum draft history can also be imported into dedicated `entry_draft_picks` and `opening_draft_picks` tables after the local draft JSON files have been scraped. Imported entry-draft rows back the `/draft/entry` API, while opening-draft rows back the `/draft/original` API.
