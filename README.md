...
Forum draft history can also be imported into dedicated `entry_draft_picks` and `opening_draft_picks` tables after the local draft JSON files have been scraped. Imported entry-draft rows back the `/draft/entry` API, including team-level summary metrics calculated from the grouped response data; skipped `draftedPlayer: null` rows stay visible in picks lists but are excluded from those summary calculations. Opening-draft rows back the `/draft/original` API.
