...
- `npm run db:import:transactions` - Import all transaction CSVs from `csv/transactions/` into database (local by default; set `USE_REMOTE_DB=true` in `.env` for remote). Updates `import_metadata.last_modified`. Supports `--season=YYYY`, `--current-only`, `--dry-run`, and `--dir=/custom/path`.
