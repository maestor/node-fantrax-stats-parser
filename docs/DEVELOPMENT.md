...
- `entry_draft_picks` and `opening_draft_picks` store FFHL forum draft history as lightweight pick rows keyed by fantasy team IDs. Entry drafts are season-scoped and back `/draft/entry`; opening draft rows have no season column and back `/draft/original`.
