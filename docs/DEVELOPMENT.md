...
- `entry_draft_picks` and `opening_draft_picks` store FFHL forum draft history as lightweight pick rows keyed by fantasy team IDs. Entry drafts are season-scoped and back `/draft/entry`, where team-level summary metrics are calculated after fetching the grouped rows and skipped `player_name NULL` placeholder rows are excluded from those summary calculations while still remaining visible in the season pick lists; opening draft rows have no season column and back `/draft/original`.
