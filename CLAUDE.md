...
- `plusMinus` uses per-dataset min/max, where the minimum can be negative; `gaa` is inverted min/max (lower is better), and `savePercent` follows the standard "higher is better" min/max normalization.
